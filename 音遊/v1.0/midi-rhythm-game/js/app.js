...
function loadPresetSong(presetId) {
  try {
    currentSongName = presetId === 'canon' ? 'Canon in D Major' : 'Für Elise';
    ...
    if (songMetaEl) songMetaEl.innerText = `Normal · ${currentLaneCount} Lanes · ${chart.totalNotes} Notes`;
    setStatus(`內建曲目：${currentSongName}`, 'success');
  } catch(e) { console.error('loadPresetSong:', e); setStatus('內建曲目載入失敗', 'error'); }
}
...
function goHome() {
  ...
  [presetModal, pauseModal, resultModal].forEach(m => m.classList.remove('active'));
  showScreen('home');
}
