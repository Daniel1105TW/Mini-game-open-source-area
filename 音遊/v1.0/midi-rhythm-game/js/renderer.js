...
class Renderer {
  ...
  _drawNotes(gameState, g, laneCount, scrollSpeed, noteMode) {
    ...
    for (const note of notes) {
      ...
      if (td < -0.35 && note.type === 'tap') continue;
      ...
      // Roll notes get a vibrating/segmented visual cue
      const isRoll = noteMode === 'roll' && note.type === 'roll';
      ...
      if (note.type === 'tap' || isRoll) {
        ...
        ctx.shadowColor = col; ctx.shadowBlur = isRoll ? 8 : 16;
        ctx.fill();
        ...
        const endPos = this._lanePos(note.lane, laneCount, Math.max(0, endProg), g);
