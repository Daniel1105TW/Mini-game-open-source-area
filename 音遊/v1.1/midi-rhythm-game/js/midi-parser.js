...
class MIDIParser {
  ...
  getTutorialChart(laneCount = 4) {
    const notes = [];
    const bgmNotes = [];

    // Step 1: Single Taps
    const tapPitches = [60, 62, 64, 65, 67, 69, 71, 72];
    tapPitches.forEach((pitch, i) => {
      const time = 1.0 + i * 0.8;
      const lane = i % laneCount;
      bgmNotes.push({ pitch, time, duration: 0.3, velocity: 90 });
      notes.push({ lane, time, type: 'tap', duration: 0, hitState: 'pending', rollHitCount: 0 });
    });

    // Step 2: Hold Notes
    const holdPitches = [72, 74, 76];
    holdPitches.forEach((pitch, i) => {
      const time = 7.8 + i * 1.6;
      const lane = i % laneCount;
      bgmNotes.push({ pitch, time, duration: 1.0, velocity: 95 });
      notes.push({ lane, time, type: 'hold', duration: 0.9, hitState: 'pending', rollHitCount: 0 });
    });

    // Step 3: Roll Notes (continuous fast hits)
    const rollPitches = [79, 81, 83];
    rollPitches.forEach((pitch, i) => {
      const time = 13.2 + i * 1.3;
      const lane = (i + 1) % laneCount;
      bgmNotes.push({ pitch, time, duration: 0.4, velocity: 100 });
      notes.push({ lane, time, type: 'roll', duration: 0, hitState: 'pending', rollHitCount: 0 });
    });

    return {
      name: '🎓 互動新手教學 (Tutorial Practice)',
      notes,
      bgmNotes,
      duration: 18,
      totalNotes: notes.length
